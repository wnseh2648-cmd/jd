...
# -------------------------------------------------
    ...
    if rate_ratio >= 70:
        conclusion_text = f"현재 <span class='highlight'>{region_name}</span> 지역은 <b>거시경제(기준금리)의 강력한 지배</b>를 받고 있습니다. 지역 내의 자체적인 매수세(거래량) 회복만으로는 가격 하방 압력을 이겨내기 어려우며, 철저히 거시경제 지표(금리 인하 등) 시그널에 맞춰 전략을 짜야 하는 <b>'거시경제 종속형 시장'</b>임을 데이터가 증명합니다."
        
    elif vol_ratio >= 50:
        conclusion_text = f"현재 <span class='highlight'>{region_name}</span> 지역은 이례적으로 외부 금리 충격보다 <b>'지역 자체의 매수세(거래량)'</b>가 가격을 견인하는 <b>'수급 주도형 시장'</b>입니다. 거시시경제가 흔들리더라도, 지역 내의 탄탄한 실거주 및 투자 수요가 가격을 강력하게 방어하거나 밀어 올리고 있음을 시사합니다."
